 ...
 * Returns null if not found or on error.
 */
export async function lookupPersonByUuid(uuid: string): Promise<Person | null> {
	try {
		const response = await fetch(
			`${GOVERNANCE_URL}/api/persons/by-uuid/${encodeURIComponent(uuid)}`,
			{
				method: 'GET',
				headers: { Accept: 'application/json' },
			}
 ...
 * Returns null if not found or on error.
 */
export async function lookupAssociationByUuid(
	uuid: string
): Promise<Association | null> {
	try {
		const response = await fetch(
			`${GOVERNANCE_URL}/api/associations/by-uuid/${encodeURIComponent(uuid)}`,
			{
				method: 'GET',
				headers: { Accept: 'application/json' },
			}
