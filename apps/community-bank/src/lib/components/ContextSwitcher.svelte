...
<style>
	...
	/* Light sidebar optimized button - uses dark colors for contrast */
	.switcher-button {
		width: 100%;
		display: flex;
		align-items: center;
		...
		background: var(--ledger-lined, #f9f6f0);
		border: 1px solid var(--border, rgba(139, 90, 60, 0.2));
	...
	.switcher-button:hover:not(:disabled) {
		background: var(--copper-light, #f4ede8);
		border-color: var(--border-strong, rgba(139, 90, 60, 0.3));
	}
	...
	.switcher-button.association {
		background: var(--olive-light, #e8ede5);
		border-color: var(--olive, #5a6b4a);
		font-weight: 600;
	}
	...
	.spinner {
		width: 20px;
		height: 20px;
		border: 2px solid var(--border-subtle, rgba(139, 90, 60, 0.12));
		border-top-color: var(--copper, #8b5a3c);
		border-radius: 50%;
