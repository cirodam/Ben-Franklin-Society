 ...
 * If a vote_session exists for this motion+meeting, uses that system.
 * Otherwise falls back to legacy vote_tally or manual counts.
 */
export function recordOutcome(input: {
	...
	if (input.action_taken === 'vote_held') {
		// Check for vote_session associated with this motion and meeting
		const sessions = listVoteSessions({
			motion_uuid: input.motion_uuid,
			meeting_uuid: input.meeting_uuid
		});
		
		if (sessions.length > 0) {
			// Use vote_session system (new approach)
			const session = sessions[0]; // Use most recent if multiple
			
			// Get vote counts from session
			...
			// Finalize the session (closes and determines outcome, advances motion)
			...
			// Fallback to legacy vote_tally system
			...
			if (tally) {
				...
				// Legacy: directly advance motion (for backward compatibility)
	...
	// Create and save meeting outcome record
	const uuid = randomUUID();
