<script lang="ts">
	...
	import AssignPersonModal from './AssignPersonModal.svelte';

	let { data, form }: { data: PageData; form: ActionData } = $props();

	const { role, association, section, parentRole, holders, childRoles, permissions, history, canManage, availablePermissions, allPeople } = $derived(data);
	...
	<div class="content">
					...
					<ul class="holders-list">
						{#each holders as holder}
							<li class="holder-item">
								...
								{#if canManage}
								<form method="POST" action="?/remove_role" use:enhance>
		...
		<section class="role-section">
			<div class="section-header-with-action">
				<h2>Permissions</h2>
				{#if canManage}
					<Button size="sm" variant="secondary" onclick={() => showPermissionsModal = true}>
						Edit Permissions
					</Button>
				{/if}
			</div>
