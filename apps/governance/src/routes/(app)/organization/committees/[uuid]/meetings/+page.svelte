	...
	<header class="header">
		<div class="header__content">
			<div class="breadcrumb">
				<a href="/organization/committees/{association.uuid}" class="breadcrumb__link">← {association.name}</a>
			</div>
			<h1 class="header__title">Committee Meetings</h1>
			<p class="header__subtitle">
				Schedule and manage meetings for {association.name}
			</p>
		</div>
		{#if canManage}
			...
			{:else}
				<div class="meeting-list">
					{#each upcomingMeetings as meeting}
						<a href="/organization/committees/{association.uuid}/meetings/{meeting.uuid}" class="meeting-card">
			...
			<section class="section">
				...
				<div class="meeting-list meeting-list--compact">
					{#each pastMeetings as meeting}
						<a href="/organization/committees/{association.uuid}/meetings/{meeting.uuid}" class="meeting-card meeting-card--compact">
							<div class="meeting-card__date">
								<div class="meeting-card__date-day">{formatDate(meeting.scheduled_at).split(',')[1].trim().split(' ')[1]}</div>
								<div class="meeting-card__date-month">{formatDate(meeting.scheduled_at).split(' ')[0]}</div>
							</div>
...
<Modal bind:open={showCreateModal} title="Schedule New Meeting">
	<form method="POST" action="?/create" use:enhance>
		<div class="form-group">
			...
			<Input id="title" name="title" bind:value={createTitle} required placeholder="Committee Meeting" />
		</div>
