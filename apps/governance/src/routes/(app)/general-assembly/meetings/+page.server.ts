...
export const load: PageServerLoad = async ({ locals, url }) => {
	...
	// Get the community association (body_uuid for meetings)
	const association = db
		.prepare(`SELECT * FROM association WHERE handle = 'community' AND type = 'community'`)
		.get() as { uuid: string; name: string; handle: string } | undefined;

	if (!association) error(500, 'Community association not found');
...
export const actions: Actions = {
	create: async ({ locals, request }) => {
		...
		// Get community association
		const association = db
			.prepare(`SELECT uuid FROM association WHERE handle = 'community' AND type = 'community'`)
			.get() as { uuid: string } | undefined;

		if (!association) return fail(500, { error: 'Community association not found' });
