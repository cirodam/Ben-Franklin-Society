...
<div class="paper-card">
	<div class="paper-card__header">
		...
		{#if canCreateVoteSession}
			<a href="/governance/vote-sessions/new?motion_uuid={motionUuid}" class="btn btn--secondary btn--sm">
	...
	{#if voteSessions && voteSessions.length > 0}
		<div class="vote-sessions-list">
			{#each voteSessions as session}
				<a href="/governance/vote-sessions/{session.uuid}" class="vote-session-card">
	...
	{:else}
		<p class="empty-message">
			...
			{#if canCreateVoteSession}
				<a href="/governance/vote-sessions/new?motion_uuid={motionUuid}">Schedule one now</a>
			{/if}
		</p>
	{/if}
</div>
