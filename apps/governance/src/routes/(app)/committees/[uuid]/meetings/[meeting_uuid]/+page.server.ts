...
export const load: PageServerLoad = async ({ params, locals }) => {
	...
	const meeting = getMeetingWithAgenda(params.meeting_uuid);
...
export const actions: Actions = {
	...
	startMeeting: async ({ params, locals }) => {
		...
		try {
			updateMeetingStatus(params.meeting_uuid, 'in_progress');
			return { success: true };
		} catch (err) {
			return fail(500, { error: (err as Error).message });
		}
	...
	completeMeeting: async ({ params, locals }) => {
		...
		try {
			updateMeetingStatus(params.meeting_uuid, 'completed');
			return { success: true };
		} catch (err) {
			return fail(500, { error: (err as Error).message });
		}
