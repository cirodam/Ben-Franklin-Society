...
export function getOidcConfig() {
	return {
		...
		clientId: getConfig('oidc:client_id') ?? 'mail',
		clientSecret: getConfig('oidc:client_secret'),
		redirectUri: getConfig('oidc:redirect_uri') ?? (env.PUBLIC_URL ? `${env.PUBLIC_URL}/oauth/callback` : 'http://localhost:5175/oauth/callback'),
	};
}
