 ...
 * Proxy to library service: list buckets accessible to user
 */
export const GET: RequestHandler = async ({ locals, fetch, cookies }) => {
	...
	try {
		...
		const response = await fetch(`${LIBRARY_URL}/api/buckets`, {
			headers: {
		...
		console.error('[mail/api/library/buckets] Error:', err);
