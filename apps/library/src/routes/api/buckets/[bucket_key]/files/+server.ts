...
import { listRootFiles } from '$lib/server/files.js';
import { getBucket } from '$lib/server/buckets.js';

/**
 * GET /api/buckets/[bucket_key]/files
...
export const GET: RequestHandler = async ({ params, locals }) => {
	...
	try {
		...
		// Verify access (user buckets use person_uuid, not acting_as_uuid)
		if (bucket.owner_type === 'user' && bucket.owner_id !== session.person_uuid) {
			throw error(403, 'Not authorized to access this bucket');
		}

		// List files
		const files = listRootFiles(bucket.id);

		return json({ files });
	} catch (err: any) {
		console.error('[library/api/buckets/files] List error:', err);
